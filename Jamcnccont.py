    ...
    def run(self):
        while self.running:
            ...
            if self.msg:
                ...
                self.ls.poll()
                x,y,z,a,b,c,u,v,w = self.ls.actual_position
                # NOTE: Ignore ABCUVW since not used for my machine
                # Subtract G5x offset
                xo,yo,zo,ao,bo,co,uo,vo,wo = self.ls.g5x_offset
                ##print(f"{xo:.3f},{yo:.3f},{zo:.3f}")
                x -= xo
                y -= yo
                z -= zo

                print(f"bbb {x:.3f},{y:.3f},{z:.3f}")
                # Subtract tool offset
                xo,yo,zo,ao,bo,co,uo,vo,wo = self.ls.tool_offset
               ## print(f"Tool offset: {xo},{yo},{zo}")
                x -= xo
                y -= yo
                z -= zo

                # Print offset coordinates
                print(f"aaa {x:.3f},{y:.3f},{z:.3f}")
                time.sleep(0.24)
